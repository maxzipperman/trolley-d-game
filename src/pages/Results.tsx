  ...
  return (
    <main className="min-h-screen container max-w-3xl py-8 space-y-8">
      ...
      {selectedPersonas.length > 0 && (
        <section className="p-4 rounded-lg border border-border bg-card motion-safe:animate-scale-in">
      ...
      <section className="p-4 rounded-lg border border-border bg-card motion-safe:animate-scale-in">
            ...
            return (
              <li key={s.id} className="flex items-center justify-between gap-3 border-b border-border/60 py-2">
                <div className="flex-1">
                  <Link
                    to={`/play?jump=${s.id}`} className="underline underline-offset-4">{s.title}</Link>
        ...
        <div className="mt-4 flex gap-2">
          <button
            ...
            className="px-4 py-2 rounded-md border border-border hover:bg-accent motion-safe:transition-colors"
          >
            Reset Game
          </button>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="text-center">
          <Link
            to="/"
            className="inline-flex items-center justify-center bg-primary text-primary-foreground font-medium rounded-md px-6 py-2 hover:bg-primary/90 motion-safe:transition-colors"
          >
            Start Over
          </Link>
        </div>
